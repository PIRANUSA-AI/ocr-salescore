    ...
    try {
      ...
      const byQuestion = (keywords: string[]) => {
        for (const k of keywords) {
          const m = fa.find((f: any) => f.question.toLowerCase().includes(k));
      ...
      const pi = matchOptions(piQ || piA.join(', ') || sw, PRODUCT_INTEREST);
      setProductInterest(pi.matched.length > 0 ? pi.matched : piA);
      if (pi.other) setOtherProduct(pi.other);

      const sQ = byQuestion(['software', 'saat ini', 'digunakan']);
      const sA = byAnswer(SOFTWARE_OPTIONS);
      const s = matchOptions(sQ || sA.join(', ') || sw, SOFTWARE_OPTIONS);
      setCurrentSoftware(s.matched.length > 0 ? s.matched : sA);
                ...
                <div className="grid grid-cols-2 gap-2">
                  {FOLLOWUP_OPTIONS.map((f) => (
                    <div key={f} className="flex items-center gap-2">
                      <Checkbox checked={followUp.includes(f)} onCheckedChange={(c) => setFollowUp(prev => c ? [...prev, f] : prev.filter(x => x !== f))} id={`q-fu-${f}`} />
